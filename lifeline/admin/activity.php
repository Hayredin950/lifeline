...
<div class="card">
    ...
    <!-- Stats -->
    <div class="grid-autofit-140 my-20">
        <div class="info-box info-box--success">
            <div class="info-box-value text-success-dark"><?php echo $statsToday; ?></div>
            <div class="info-box-label">Today's Events</div>
        </div>
        <div class="info-box info-box--info">
            <div class="info-box-value text-info-dark"><?php echo $statsLogins; ?></div>
            <div class="info-box-label">Logins Today</div>
        </div>
        <div class="info-box info-box--danger">
            <div class="info-box-value text-danger-dark"><?php echo $statsFailed; ?></div>
            <div class="info-box-label">Failed Logins</div>
        </div>
        <div class="info-box info-box--warning">
            <div class="info-box-value text-warning-dark"><?php echo $statsDeletes; ?></div>
            <div class="info-box-label">Total Deletes</div>
        </div>
    </div>

    <!-- Filters -->
    <form method="GET" class="flex gap-10 flex-wrap items-end mb-20">
        <div class="form-group mb-0 flex-1 minw-150">
    ...
    <?php if (count($logs) > 0): ?>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td class="nowrap fs-85"><?php echo htmlspecialchars($log['created_at']); ?></td>
                    <td><?php echo $log['user_email'] ? htmlspecialchars($log['user_email']) : '<span class="text-muted">System</span>'; ?></td>
                    <td>
                        <?php
                        $actionClasses = [
                            'login' => 'text-info-dark', 'login_failed' => 'text-danger-dark', 'delete' => 'text-danger-dark',
                            'create' => 'text-success-dark', 'update' => 'text-warning-dark', 'password_reset' => 'text-purple-dark'
                        ];
                        $actionClass = $actionClasses[$log['action']] ?? 'text-muted';
                        ?>
                        <span class="fw-600 <?php echo $actionClass; ?>"><?php echo htmlspecialchars($log['action']); ?></span>
                    </td>
                    <td><?php echo htmlspecialchars($log['entity_type']); ?></td>
                    <td><?php echo $log['entity_id'] ?? '-'; ?></td>
                    <td class="ellipsis maxw-200 fs-85">
                        <?php
                        if ($log['old_values']) {
                            echo '<span class="text-danger-dark" title="' . htmlspecialchars($log['old_values']) . '">Old: ' . htmlspecialchars(substr($log['old_values'], 0, 60)) . '</span>';
                        }
                        if ($log['new_values']) {
                            echo '<span class="text-success-dark" title="' . htmlspecialchars($log['new_values']) . '"> New: ' . htmlspecialchars(substr($log['new_values'], 0, 60)) . '</span>';
                        }
                        ?>
                    </td>
                    <td class="fs-85"><?php echo htmlspecialchars($log['ip_address'] ?? ''); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
